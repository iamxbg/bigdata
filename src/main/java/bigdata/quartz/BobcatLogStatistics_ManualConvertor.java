...
public class BobcatLogStatistics_ManualConvertor {
	
	private static Logger logger=LogManager.getLogger(BobcatLogStatistics_ManualConvertor.class);

	@Test
	public void get_LogStatistics() throws InterruptedException, ParseException{

		Date start=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2017-04-27 12:00:00");
		Date end=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2017-04-28 09:00:00");
		...
		for(int i=0;i<tsList.size();i++){
			...
			Thread t=new Thread(new Runnable() {
				...
				public void run() {
					...
					LogConvertor convertor=new Bobcat_LogConvertor(startTime, endTime);
