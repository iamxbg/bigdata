...
@Service
public class Bobcat2ServiceImpl implements BobcatService{
	...
	private static final String CL="lhbobcat";
	...
	public Map<Object,  Long> countLasOneHourGroupByMinute(String startTime, String endTime) throws BaseException, InterruptedException {
		...
		try{
			String sql=new StringBuilder("select * from lhbbc.dcslog where log_date >='"+startTime+"' and log_date <'"+endTime+"'").toString();
	...
	public long countOfToday(String startTime, String endTime) throws BaseException, InterruptedException {
		...
		StringBuilder sb=new StringBuilder("select count(_id) as count from lhbbc.dcslog where log_date>='")
				.append(startTime)
				.append("' and log_date <'")
				.append(endTime)
				.append("' ");
		...
		while(cur.hasNext()){
			count=(long) cur.getNext().get("count");
			 logger.info("count of today-LH:"+count);
		}
		return count;
	}
